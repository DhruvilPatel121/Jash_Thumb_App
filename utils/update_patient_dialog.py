...
class UpdatePatientDialog(QFrame):
    ...
    def setup_ui(self):
        ...
        consultancy_row = QHBoxLayout()
        consultancy_row.addWidget(self.consultancy_input)
        consultancy_row.addWidget(self.treatment_checkbox)
        consultancy_row.addStretch()
        form_layout.addRow("Consultancy Fee", consultancy_row)
        ...
        self.treatment_checkbox.installEventFilter(self)
        self.payment_input.installEventFilter(self)
        self.add_paid_days_input.installEventFilter(self)
        ...
        if event.type() == QEvent.Type.KeyPress:
            if event.key() == Qt.Key.Key_Tab:
                if obj == self.age_input:
                    ...
                    self.treatment_checkbox.setFocus()
                    return True
                elif obj == self.treatment_checkbox:
    ...
    def show_form(self, patient):
        ...
        if isinstance(treatment_start_today, str):
            treatment_start_today = treatment_start_today.lower() in ("true", "1", "yes")
        self.treatment_checkbox.setChecked(bool(treatment_start_today))
    ...
    def save_updated_patient(self):
        ...
        try:
            old_treatment_status = bool(self.patient_to_update.get("treatment_start_from_today", False))
